...
const Footer = () => {
  ...
  return (
    <footer className="bg-gray-900 text-white border-t border-gray-800 py-12 mt-12">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          {/* About */}
          <div>
            <h3 className="text-lg font-bold mb-4">{language === 'ar' ? 'عن المتجر' : 'About Us'}</h3>
            <p className="text-gray-400 text-sm mb-4">
              {language === 'ar' 
                ? 'متجر إلكتروني احترافي يقدم أفضل المنتجات بأسعار تنافسية'
                : 'Professional e-commerce store offering the best products at competitive prices'}
            </p>
          ...
          {/* Legal */}
          <div>
            <h3 className="text-lg font-bold mb-4">{language === 'ar' ? 'معلومات قانونية' : 'Legal'}</h3>
            <ul className="space-y-2">
              <li>
          ...
          <div>
            ...
            {settings.phone_number && (
              <p className="text-gray-400 text-sm mb-2">
                <a href={`tel:${settings.phone_number}`} className="hover:text-white transition">
                  {settings.phone_number}
                </a>
              </p>
            )}
            {settings.whatsapp_number && (
              <button
                ...
                className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 space-x-reverse transition text-sm"
              >
